 ...
 *	num ：0表示不滚动,1表示滚动
 */
function marquee_new(container,content,step,len,num){
	if(marquee.handler)clearInterval(marquee.handler);	
	var temp1 = content.replace(/[^\x00-\xff]/g, "**");
	var temp2 = temp1.substring(0, len);
	var x_length = temp2.split("\*").length - 1;
	var hanzi_num = x_length / 2;
	len = len - hanzi_num;
	var res = content.substring(0, len);
	if(num === 0) {
		...
		if(len < content.length) {
			...
			span.style.width = content.length*step + "px";
			container.innerHTML = "";
			container.appendChild(span);
			container.style.overflow = "hidden";
			var alls = span.offsetLeft+span.offsetWidth;
			var times = Math.ceil(alls / step); 
			var i = 0;
			marquee.handler = setInterval(function(){
				if(i != times){
					span.style.left = -i*step+"px";
				}else{
					container.innerHTML = "<marquee scrollLeft='1' behavior='scroll' direction='left' scrollamount='"+step+"' scrolldelay='200'>" + content + "</marquee>";
					clearInterval(marquee.handler);	
				}
				i++;
			},200);
		}else{
